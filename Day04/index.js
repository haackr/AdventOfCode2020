...
const passportStrings = inputString
  .split(/\n\s*\n/gim)
...
function convertPassportsToObjects(passportArray) {
  return passportArray.map((passportString) =>
    passportString.reduce((acc, cur) => {
      const key = cur.match(/[a-z]+(?=:)/gi)[0];
      const value = cur.match(/(?<=:).*/gi)[0];
      acc[key] = value;
      return acc;
    }, {})
  );
...
function isPassportValidStrict(passport) {
  for (const field of REQUIRED_FIELDS) {
    ...
    if (
      field === "ecl" &&
      !/^(amb|blu|brn|gry|grn|hzl|oth)$/.test(fieldValue)
    ) {
...
console.log(
  `Part 1 solution: ${countValidPassports(
    passportList,
    isPassportValid
  )} out of ${passportList.length}`
);
console.log(
  `Part 2 solution: ${countValidPassports(
    passportList,
