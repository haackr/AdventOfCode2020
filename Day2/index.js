...
function extractPasswordDataFromString(inputString) {
  const minOccurance = Number(inputString.match(/\d+(?=-)/gim)[0]);
  const maxOccurance = Number(inputString.match(/(?<=-)\d+/gim)[0]);
  const char = inputString.match(/[a-z]{1}(?=:)/gim)[0];
  const password = inputString.match(/(?<=:\s)[a-z]+/gim)[0];
  return {
    minOccurance,
    maxOccurance,
    char,
...
function validatePasswordPart2({
  password,
  minOccurance: posOne,
  maxOccurance: posTwo,
  char,
...
console.log(
  `Part 1 solution: ${
    passwordList.map(validatePassword).filter(Boolean).length
  }`
);
console.log(
  `Part 2 solution: ${
    passwordList.map(validatePasswordPart2).filter(Boolean).length
  }`
);
