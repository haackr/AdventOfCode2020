...
function findPosition(boardingPass) {
  const row = parseInt(
    boardingPass.substring(0, 7).replace(/[FB]/g, (c) => (c === "B" ? 1 : 0)),
    2
  );
  // console.log(row);

  const col = parseInt(
    boardingPass.substring(7, 10).replace(/[RL]/g, (c) => (c === "R" ? 1 : 0)),
    2
  );
  // console.log(col);
...
function findMissingPass(boardingPasses) {
  ...
  const filteredIds = boardingPasses
    .filter((pass) => pass.row !== minRow && pass.row !== maxRow)
    .sort((a, b) => a.id - b.id)
    .map((pass) => pass.id);
...
console.log(
  `Part 1 solution: ${Math.max(...boardingPasses.map((pass) => pass.id))}`
);
