...
function countUniqueInGroup(groupString){
    ...
    const uniqueAnswers = groupString.split('').filter((char, i, arr) => arr.indexOf(char) === i);
    // console.log(uniqueAnswers.join(''));
    return uniqueAnswers.length;
}

const questionsSum = answers.reduce((acc, cur) => acc += countUniqueInGroup(cur), 0);
...
const everyoneQuestionsSum = answers.reduce((acc, cur) => acc += countQuestionsEveryoneAnswered(cur), 0);
